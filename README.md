...
- boto3 / pypdf
- SQLModel（開発・本番ともに PostgreSQL、`DATABASE_URL` で SQLite / MySQL にも切替可）

### インフラ
- Docker Compose（backend / frontend / postgres の3サービス）
...
```bash
git clone https://github.com/micchy466/ai-proofreader.git
cd ai-proofreader
cp .env.example .env   # .env を編集し AWS 認証情報を設定

docker compose up      # バックエンド + フロントエンド + PostgreSQL を一括起動
```
...
| postgres | 5432 | PostgreSQL 16（dev/prod 揃え） |

ソースコードは volume マウントされているので、編集すると自動的にホットリロードされます。

```bash
docker compose logs -f backend    # バックエンドログを追跡
docker compose down               # 停止
docker compose down -v            # DB も含めてリセット
```

### ローカル実行（Docker を使わない場合）

<details>
<summary>venv + npm で直接起動する手順</summary>

事前準備: Python 3.12+ / Node.js 20.16+
...
cp .env.example .env  # DATABASE_URL=sqlite:///./data/proofreader.db を設定
...
この場合、DB は SQLite（`data/proofreader.db`）になります。
...
```
ai-proofreader/
├── docker-compose.yml      # 3サービス定義（backend / frontend / postgres）
